...
class Ge25519_niels(ct.Structure):
    _fields_ = [("ysubx", FE), ("xaddy", FE), ("t2d", FE)]


class Ge25519_pniels(ct.Structure):  # projective Cached / Deuis
    _fields_ = [("ysubx", FE), ("xaddy", FE), ("z", FE), ("t2d", FE)]
...
class XmrCtKey(ct.Structure):
    _fields_ = [("dest", XmrKey), ("mask", XmrKey)]
...
class XmrBoroSig(ct.Structure):
    _fields_ = [("s0", XmrKey64), ("s1", XmrKey64), ("ee", XmrKey)]


class XmrRangeSig(ct.Structure):
    _fields_ = [("asig", XmrBoroSig), ("Ci", XmrKey64)]
...
class union_anon_3(ct.Union):
    ...
    _fields_ = [("wide", ct.c_uint64 * 16), ("narrow", ct.c_uint32 * 32)]
...
class struct_anon_7(ct.Structure):
    __slots__ = ["type", "ctx"]
    _fields_ = [("type", ct.c_int), ("ctx", UnionHasher)]
