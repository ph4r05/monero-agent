...
_TWO = TWO
_EIGHT = EIGHT
_INV_EIGHT = INV_EIGHT
_MINUS_ONE = MINUS_ONE
_MINUS_INV_EIGHT = MINUS_INV_EIGHT
_XMR_H = XMR_H
_XMR_HP = XMR_HP
_BP_GI_PRE = BP_GI_PRE
_BP_HI_PRE = BP_HI_PRE
_BP_TWO_N = BP_TWO_N
_BP_IP12 = BP_IP12
...
def copy_key_ifex(dst, src):
    if dst is None:
        return src
    return _copy_key(dst, src)


def copy_vector(dst, src):
    for i in range(len(src)):
        _copy_key(dst[i], src[i])
    return dst
...
def _hadamard(a, b, dst=None):
    dst = _ensure_dst_keyvect(dst, len(a))
    for i in range(len(a)):
        _sc_mul(_tmp_bf_1, a.to(i), b.to(i))
        dst.read(i, _tmp_bf_1)
        _gc_iter(i)
    return dst


def _hadamard2(a, b, dst=None):
    dst = _ensure_dst_keyvect(dst, len(a))
    for i in range(len(a)):
        _add_keys(dst[i], a[i], b[i])
        _gc_iter(i)
    return dst
