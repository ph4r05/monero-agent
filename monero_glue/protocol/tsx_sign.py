...
class TsxSigner(object):
    ...
    async def setup(self, msg: MoneroTransactionInitRequest):
        self.creds = await twrap.monero_get_creds(
            self.ctx, msg.address_n or (), msg.network_type
        )

    async def sign(self, ctx, msg: MoneroTransactionSignRequest):
        """
    ...
    async def tsx_init(self, tsx_data: MoneroTransactionData):
        """
    ...
    async def tsx_set_input(self, msg: MoneroTransactionSetInputRequest):
        """
    ...
    async def tsx_input_vini(self, msg: MoneroTransactionInputViniRequest):
        """
    ...
    async def tsx_set_output1(self, msg: MoneroTransactionSetOutputRequest):
        """
    ...
    async def tsx_all_out1_set(self, msg: MoneroTransactionAllOutSetRequest = None):
        """
    ...
    async def tsx_mlsag_done(self, msg: MoneroTransactionMlsagDoneRequest = None):
        """
    ...
    async def tsx_sign_input(self, msg: MoneroTransactionSignInputRequest):
        """
    ...
    async def tsx_sign_final(self, msg: MoneroTransactionFinalRequest = None):
        """
        Final message.
...
class TTransactionBuilder(object):
    ...
    def hmac_key_txin(self, idx):
        ...
        return crypto.keccak_2hash(
            self.key_hmac + b"txin" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def hmac_key_txin_comm(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_hmac + b"txin-comm" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def hmac_key_txdst(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_hmac + b"txdest" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def hmac_key_txout(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_hmac + b"txout" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def hmac_key_txout_asig(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_hmac + b"txout-asig" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def enc_key_txin_alpha(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_enc + b"txin-alpha" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def enc_key_spend(self, idx):
        ...
        """
        return crypto.keccak_2hash(
            self.key_enc + b"txin-spend" + xmrserialize.dump_uvarint_b(idx)
        )
    ...
    def enc_key_cout(self, idx=None):
        ...
        """
        return crypto.keccak_2hash(
            self.key_enc + b"cout" + (xmrserialize.dump_uvarint_b(idx) if idx else b"")
        )
    ...
    async def init_transaction(self, tsx_data, tsx_ctr):
        ...
        await self.tx_prefix_hasher.ar.message_field(
            self.tx, xmrtypes.TransactionPrefix.MFIELDS[0]
        )
        await self.tx_prefix_hasher.ar.message_field(
            self.tx, xmrtypes.TransactionPrefix.MFIELDS[1]
        )
        await self.tx_prefix_hasher.ar.container_size(
            self.num_inputs(), xmrtypes.TransactionPrefix.MFIELDS[2][1]
        )
        ...
        return MoneroTransactionInitAck(in_memory=self.in_memory(), hmacs=hmacs)
    ...
    def precompute_subaddr(self, account, indices):
        ...
        monero.compute_subaddresses(
            self.trezor.creds, account, indices, self.subaddresses
        )
    ...
    async def set_input(self, src_entr):
        ...
        vini = xmrtypes.TxinToKey(
            amount=src_entr.amount, k_image=crypto.encodepoint(ki_real)
        )
    ...
    async def range_proof(self, idx, dest_pub_key, amount, amount_key):
        ...
        if self.use_bulletproof:
            ...
            C, mask, rsig = ring_ct.prove_range(
                amount, last_mask, backend_impl=True, byte_enc=True
            )
    ...
    async def set_out1(self, dst_entr, dst_entr_hmac):
        ...
        if self.out_idx == 0:
            await self.tx_prefix_hasher.ar.container_size(
                self.num_dests(), xmrtypes.TransactionPrefix.MFIELDS[3][1]
            )
        ...
        return MoneroTransactionSetOutputAck(
            tx_out=await misc.dump_msg(tx_out),
    ...
    async def all_out1_set(self):
        ...
        await self.tx_prefix_hasher.ar.message_field(
            self.tx, xmrtypes.TransactionPrefixExtraBlob.MFIELDS[4]
        )  # extra
        ...
        return MoneroTransactionAllOutSetAck(
            extra=self.tx.extra, tx_prefix_hash=self.tx_prefix_hash, rv=rv_pb
        )
